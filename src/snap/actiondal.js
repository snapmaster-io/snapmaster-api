...
// 
// exports:
//   createAction(userId, definition, private): create a action in a user's account using the definition
...
exports.createAction = async (userId, definition, private = false) => {
  try {
    ...
    const action = parseDefinition(account, definition, private);
...
const parseDefinition = (account, definition, privateFlag) => {
  try {
    ...
    if (action.actions.find(a => a.indexOf(' ') >= 0)) {
